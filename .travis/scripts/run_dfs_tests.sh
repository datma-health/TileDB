...
if [[ $INSTALL_TYPE != basic ]]; then
  ...
  if [[ $INSTALL_TYPE == hdfs ]]; then
    ...
    echo "az storage test" &&  $TILEDB_BUILD_DIR/test/test_azure_blob_storage --test-dir "az://$AZURE_CONTAINER_NAME@$AZURE_ACCOUNT_NAME.blob.core.windows.net/travis_azure_blob_storage_test" &&
