...
if [[ $INSTALL_TYPE != basic ]]; then
    ...
    cat $TRAVIS_BUILD_DIR/.travis/resources/gcs/GCS.json
    tiledb_utils_tests "gs://nalini-oda-test/travis_unit_test" &&
		time $TRAVIS_BUILD_DIR/examples/run_examples.sh "gs://nalini-oda-test/travis_test"
